...
export abstract class ArrayState<T> extends BaseState<T[]> {
  ...
  /**
   * 	Push item to array in state if not exists in state
   *	@public
   *	@param {Generic} item - Item needs to push
   */
  @Action
  public pushItemIfNotExists(item: T): void {
    if (
      this.data &&
      this.data.filter((i) => this.compareItems(i, item)).length === 0
    ) {
      this.pushItem(item);
    }
  }
